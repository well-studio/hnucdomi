...
class Ajax extends Controller{
	...
	private function query_domi() {
		$out = Db::query('SELECT `id` as `stu_id`,`name` as `stu_name`, `academy` as `stu_academy`, `class` as `stu_class`, `domi_num` as `stu_domiNum`, `bed_num` FROM `stu_info` WHERE (`name` = ? OR `id` = ?) AND (`bed_num` > 0)', [input('post.stu_info'), intval(input('post.stu_info'))]);
		for($i=0,$cnt=count($out); $i<$cnt; $i++) {
			$t = Db::query('SELECT `name` as `stu_name` FROM `stu_info` WHERE `domi_num` = ?', [$out[$i]['stu_domiNum']]);
			$out[$i]['stu_domiNum'] = $out[$i]['stu_domiNum'].' # '.$out[$i]['bed_num'];
			$out[$i]['domi_mate'] = [];
